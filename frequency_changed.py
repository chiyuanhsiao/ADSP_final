...
def main(input_file):
    ...
    sf.write('frequency_changed.wav', y_doubled, sr)
    ...
    plt.subplot(4, 1, 2)
    librosa.display.specshow(librosa.amplitude_to_db(np.abs(D), ref=np.max), sr=sr, hop_length=hop_length, x_axis='time', y_axis='log')
    plt.colorbar(format='%+2.0f dB')
    plt.title('Original Spectrogram')

    plt.subplot(4, 1, 3)
    librosa.display.waveshow(y_doubled, sr=sr)
    plt.title('Frequency-Changed Wave')
    ...
    plt.colorbar(format='%+2.0f dB')
    plt.title('Frequency-Changed Spectrogram')
